...
absolute_filter_biom_table <- function(biom_table, minimum_count) {
  ...
  withProgress(message = 'Filtering Full OTU table', value = 0, {
    incProgress(0.1, detail = "Filtering OTUs (absolute)")
    full_biom_matrix <- as(biom_data(biom_table), "matrix")
    incProgress(0.5, detail = "Filtering OTUs (absolute)")
    ...
    output$X.SampleID <- row.names(output)
