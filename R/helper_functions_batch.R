...
absolute_filter_biom_table <- function(biom_table, minimum_count) {
    full_biom_matrix <- as(biom_data(biom_table), "matrix")
    max_values <- apply(full_biom_matrix, 1, max)
    filtered <- full_biom_matrix[max_values >= minimum_count, ]
    output <- as.data.frame(t(filtered))
    colnames(output) <- paste("OTU", colnames(output), sep = "_")
    output$Description <- row.names(output)
    row.names(output) <- NULL
    out_list <- list(output, filtered)
  return(out_list)
}

# Split a combined BIOM table into your experiment-specific one
split_into_experiment <- function(biom_table, mapping, compare, donor, recipient, post_fmt) {
    mapping_experiment <- mapping %>% filter(get(compare, envir = as.environment(mapping)) %in% c(donor, recipient, post_fmt))
    samples <- mapping_experiment$Description
    experiment_only <- filter(biom_table, Description %in% samples)
    samples_out <- experiment_only$Description
    map_out <- filter(mapping, Description %in% samples_out)
    output <- list(experiment_only, map_out)
  return(output)
}

# Normalize OTU counts for your OTU table, and impose a minimum filter
normalize_and_filter <- function(biom_table, min_fraction) {
    table_only <- select(biom_table, -Description)
    ...
    otu_filtered$Description <- biom_table$Description
    output <- list(otu_filtered, table_only, normalized)
  return(output)
}


# Define function to get tables that are specific for each given condition
...
# Define function to reorder tables with a given value first
...
# Define a function that returns a table of OTUs and the fraction of samples in which they have non-zero values
...
# Define a function to produce tables populated only by OTUs that have non-zero counts in a given fraction
# of all of the samples for that condition
...
# Define a function that will collapse a table by shared taxonomy and average the mean/median OTU count & differences
...
# Define a function that will take a vector and return the mean of the vector only if it is a vector of numbers
# but returns the first instance of the vector when it is not a number
...
visualize_metrics <- function(N_donor, N_recipient, N_postFMT, FMT_donor, FMT_recipient, N_unique_postfmt, N_shared_postfmt, title_text, output_name) {
