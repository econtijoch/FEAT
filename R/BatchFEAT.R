batchFEAT <- function(biom_table, mapping_file, FMT_pairs, input_params, output_dir) {
  ...
  source('~/GitHub/FMT Efficacy Analysis Toolkit/helper_functions_batch.R')
  options(digits = 4)
  ...
  cat("Filtering OTUs with less than", min_count, "counts per OTU...\n")
  
  #Filter OTU table to exclude OTUs with less than the given number of counts per OTU.
  absolute_filtered <- absolute_filter_biom_table(biom, min_count)
  
  filtered <- absolute_filtered[[1]]
  absolute_pre_filtered <- t(absolute_filtered[[2]]) # For QC
  
  # How many OTUs survived
  n_otus_after_absolute_filter <- ncol(filtered) - 1
  cat("# of OTUs that pass this filter:", n_otus_after_absolute_filter, '\n')
  
  transplant_pairs <- read.delim(file = FMT_pairs, header = TRUE)
  
  table_out_items <- c("Comment", "TransplantID", "Input OTU Table", "# OTUs in raw table", "Minimum Absolute Count Filter", 
  ...
  for (i in 1:nrow(transplant_pairs)) {
    ...
    experiment_specific <- split_into_experiment(filtered, mapping, metadata_category, donor, recipient, post_fmt)
    write.csv(experiment_specific, paste(output_name, "experiment_specific_table.csv", sep = "_"), row.names = FALSE)
    
    n_donor_samples <- nrow(experiment_specific[[2]] %>% filter(get(metadata_category, envir = as.environment(.)) == donor))
    n_recipient_samples <- nrow(experiment_specific[[2]] %>% filter(get(metadata_category, envir = as.environment(.)) == recipient))
    n_post_fmt_samples <- nrow(experiment_specific[[2]] %>% filter(get(metadata_category, envir = as.environment(.)) == post_fmt))
    ...
    normalized_filtered_list <- normalize_and_filter(experiment_specific[[1]], min_abundance)
    ...
    full_table <- merge(normalized_filtered, mapping[,c(-1,-2,-3)])
    ...
    donor_only_table <- get_condition_only_table_function(full_table, metadata_category, donor)
    write.csv(donor_only_table, paste(output_name, "donor_only_table.csv", sep = "_"), row.names = FALSE)
    recipient_only_table <- get_condition_only_table_function(full_table, metadata_category, recipient)
    write.csv(recipient_only_table, paste(output_name, "recipient_only_table.csv", sep = "_"), row.names = FALSE)
    post_fmt_only_table <- get_condition_only_table_function(full_table, metadata_category, post_fmt)
    ...
    Item <- c("Comment", "TransplantID", "Input OTU Table", "# OTUs in raw table", "Minimum Absolute Count Filter", 
              "# OTUs After Minimum Absolute Count Filter", "Mapping File", "Metadata Category to Select FMT Details", 
              "Donor", "# Donor Samples", "Recipient", "# Recipient Samples", "Post-FMT Recipient", "# Post-FMT Recipient Samples", "Minimum Relative Abundance Filter", "# OTUs After Minimum Relative Abundance Filter", 
              "Fleeting OTU Filter Threshold", "# OTUs After Fleeting OTU Filter (Final # OTUs)", "Comparison Metric", 
              "N_don: # specific & unique to donor", "N_rec: # specific & unique to recipient", "N_post_fmt: # specific to post-FMT", "N_post_fmt_excl: # specific to post-FMT, excluding shared and unique",
              ...
              "FMT_don_excl: # from donor in post-FMT (excluding shared & unique)", "D_Frac_FMT_excl: proportion of donor in post-FMT (excluding shared & unique)", "FMT_FracD_excl: proportion of post-FMT from donor (excluding shared & unique)", 
              "FMT_rec_excl: # from recipient in post-FMT (excluding shared & unique)", "R_Frac_FMT_excl : proportion of recipient in post-FMT (excluding shared & unique)", "FMT_FracR_excl: propotion of post-FMT from recipient (excluding shared & unique)")
    Value <- c(comment, transplant_id, biom_table, n_otus_starting, min_count, 
               n_otus_after_absolute_filter, mapping_file, metadata_category, 
    ...
    metric_plot <- visualize_metrics(N_otus_unique_donor, N_otus_unique_recipient, N_otus_post_fmt_full, FMT_don, FMT_rec, N_otus_unique_post_fmt, N_otus_shared_throughout, output_name, paste('Full', output_name, sep = "_"))
    metric_plot_excl <- visualize_metrics(N_otus_unique_donor, N_otus_unique_recipient, N_otus_post_fmt_excl, FMT_don, FMT_rec, 0, 0, output_name, paste('Excl', output_name, sep = "_"))
    
    ## QC Tables & Metrics
    
    QC_starting_OTU_distribution <- colSums(absolute_pre_filtered)
    QC_starting_OTU_distribution_plot <- qplot(QC_starting_OTU_distribution, xlab = 'Total OTU Representation (Count)', geom='density', main = "Unfiltered Raw OTU Table Count Distribution")
    pdf(paste("QC_starting_OTU_distribution_plot", output_name, '.pdf', sep = '_'), width = 7, height = 7)
    print(QC_starting_OTU_distribution_plot)
    dev.off()
    
    QC_absolute_filtered_OTU_distribution <- colSums(filtered[,-length(filtered)])
    QC_absolute_filtered_OTU_distribution_plot <- qplot(QC_absolute_filtered_OTU_distribution, xlab = 'Total OTU Representation (Count)', geom='density', main = "Absolute Abundance Filtered OTU Table Count Distribution")
    ...
    QC_starting_Sample_Depth_distribution <- rowSums(absolute_pre_filtered)
    QC_starting_Sample_Depth_distribution_plot <- qplot(QC_starting_Sample_Depth_distribution, geom="density", xlab = "Sequences Per Sample", ylab = "Count", main = "Unfiltered Raw OTU Table Sample Representation")
    pdf(paste("QC_starting_Sample_Depth_distribution_plot", output_name, '.pdf', sep = '_'), width = 7, height = 7)
    print(QC_starting_Sample_Depth_distribution_plot)
    dev.off()
    
    QC_absolute_filtered_Sample_Depth_distribution <- rowSums(filtered[,-length(filtered)])
    QC_absolute_filtered_Sample_Depth_distribution_plot <- qplot(QC_absolute_filtered_Sample_Depth_distribution, geom="density", xlab = "Sequences Per Sample", ylab = "Count", main = "Absolute Abundance Filtered OTU Table Sample Representation")
