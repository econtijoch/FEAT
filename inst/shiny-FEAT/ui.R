             ...
             fluidRow(
               column(6,
					  ...
					  h4("Numbers of OTUs in each sample set"),
								...
								uiOutput("N_P_Shared"),
								bsPopover('N_P_Shared', title = HTML(paste("N", tags$sub("Post-FMT (Shared)"), sep = "")), "Number of OTUs that are reliably present in both donor AND recipient samples throughout.")
								)),
				column(6, 
					 wellPanel(style = 'overflow:hidden;background-color:white',
					 h4("FMT Metrics"),
					 uiOutput("N_P_Donor"),
					 bsPopover("N_P_Donor", title = HTML(paste("N", tags$sub("Donor | Post-FMT"), sep = "")), content = "The number of OTUs in the post-transplant samples that came from the donor."),
					 uiOutput("D_Engraft"),
					 bsPopover('D_Engraft', title = HTML(paste("D", tags$sub("Engraft"), sep = "")), content = HTML(paste("The proportion of the donor microbiota that engrafts into the recipient.", tags$img(src= 'D-engraft.png'), sep = ""))),
					 uiOutput("N_P_Recipient"),
					 bsPopover("N_P_Recipient", title = HTML(paste("N", tags$sub("Recipient | Post-FMT"), sep = "")), content = "The number of OTUs in the post-transplant samples that came from the recipient."),
					 uiOutput("R_Persist"),
					 bsPopover('R_Persist', title = HTML(paste("R", tags$sub("Persist"), sep = "")), content = HTML(paste("The proportion of the recipient microbiota that persists after the transplant.", tags$img(src = 'R-persist.png'), sep = ""))),					 
					 uiOutput("P_Donor"),
					 bsPopover("P_Donor", title = HTML(paste("P", tags$sub("Donor"), sep = "")), content = HTML(paste("The proportion of OTUs in the post-transplant samples that is derived from the donor.", tags$img(src = 'P-donor.png'), sep = ""))),
					 uiOutput("P_Recipient"),
					 bsPopover("P_Recipient", title = HTML(paste("P", tags$sub("Recipient"), sep = "")), content = HTML(paste("The proportion of OTUs in the post-transplant samples that is derived from the recipient.", tags$img(src = 'P-recipient.png'), sep = ""))),
					 uiOutput("P_Unique"),
					 bsPopover("P_Unique", title = HTML(paste("P", tags$sub("Unique"), sep = "")), content = HTML(paste("The proportion of OTUs in the post-transplant samples that are neither in the donor NOR in the recipient.", tags$img(src = 'P-unique.png'), sep = ""))),
					 uiOutput("P_Shared"),
					 bsPopover("P_Shared", title = HTML(paste("P", tags$sub("Shared"), sep = "")), content = HTML(paste("The proportion of OTUs in the post-transplant samples that are shared in both the donor AND the recipient.", tags$img(src = 'P-shared.png'), sep = "")))))
             ),
             hr(),
